...
import {
  getUserStats,        type UserStats,
  getBarbershopStats,  type BarbershopStats,
  getTransactionStats, type TransactionStats,
  getAdminLoginLogs,   type AdminLoginLog,
...
export default function AdminDashboard() {
  ...
  const [shopStats,   setShopStats]   = useState<BarbershopStats | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      getBarbershopStats().then(setShopStats),
      getTransactionStats().then(setTxStats),
      getAdminLoginLogs().then(r => setLoginLogs(r.data.slice(0, 5))),
      getUnreadCount().then(r => setUnreadCount(r.data.data.count)),
    ]).finally(() => setLoading(false));
  }, []);

  const roleData = userStats ? [
    { color: "#EF9F27", label: "Owner",    value: userStats.owners   },
    { color: "#639922", label: "Barber",   value: userStats.barbers  },
    { color: "#378ADD", label: "Customer", value: userStats.customers },
  ] : [];
  const totalUsers = userStats?.total ?? 0;

  const latestActivities = loginLogs.map(log => ({
    id:        log.id,
    user:      log.user,
    email:     log.email,
    activity:  log.action,
      ...
      <div className="w-full space-y-6 lg:space-y-8">
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:items-stretch">
          ...
          <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              { icon: Users,    title: "Total Users",    value: userStats?.total          ?? 0, accent: "#F5A623", compact: true,  fontSize: "clamp(1.25rem, 4vw, 1.875rem)" },
              { icon: Store,    title: "Barbershops",    value: shopStats?.total          ?? 0, accent: "#F5A623", compact: true,  fontSize: "clamp(1.25rem, 4vw, 1.875rem)" },
              { icon: Calendar, title: "Total Bookings", value: txStats?.booking_count   ?? 0, accent: "#F5A623", compact: true,  fontSize: "clamp(1.25rem, 4vw, 1.875rem)" },
              { icon: Wallet,   title: "Total Balance",  value: txStats?.total_revenue   ?? 0, accent: "#F5A623", compact: false, fontSize: "clamp(1rem, 3vw, 1.375rem)"    },
          ...
          <div className="bg-[#141414] border border-white/10 rounded-xl p-5 flex flex-col items-center lg:w-[400px] shrink-0">
            ...
            <div className="flex flex-col gap-2 mt-5 self-start w-full">
              {roleData.map(({ color, label, value }) => (
